...
function getCellClasses(row: number, col: number): string[] {
	...
	if (col === 2 || col === 5) classes.push('border-r-[2px]', 'border-r-base-content')
	if (row === 2 || row === 5) classes.push('border-b-[2px]', 'border-b-base-content')
			...
			>
				...
				<template v-if="!isPaused">
					...
					<span
						...
						:class="{
							'animate-cell-pop':
								game.lastFilledCell.value?.row === r - 1 &&
								game.lastFilledCell.value?.col === c - 1,
							'animate-cell-shake':
								game.lastErrorCell.value?.row === r - 1 &&
								game.lastErrorCell.value?.col === c - 1
						}"
					>
